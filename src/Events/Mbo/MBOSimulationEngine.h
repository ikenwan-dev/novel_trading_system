...
void MBOSimulationEngine<Strategy>::route_internal_event(const EventV2 &ev) {
  std::visit(overloaded{
      [&](const CreateOrderEvent& /*payload*/) { virtual_exchange_.on_update(ev); },
      [&](const CancelOrderEvent& /*payload*/) { virtual_exchange_.on_update(ev); },
      ...
      [&](const AckFillOrderEvent& payload) {
          oms_.ack_fill_order(payload.order_id, payload.filled_qty, payload.price);
          strategy_.on_order_filled(payload.order_id, payload.filled_qty, payload.price);
      }
  }, ev.payload);
}

template <typename Strategy> 
void MBOSimulationEngine<Strategy>::run() {
    ...
    while (reading_feed || !network_sim_.get_event_queue().empty()) {
        ...
        if (reading_feed) {
            ...
            if (!consumer_.try_poll(msg)) continue; 
        ...
        // Process all internal events that occurred BEFORE or AT the incoming market data timestamp
