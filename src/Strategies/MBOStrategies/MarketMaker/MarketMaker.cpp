...
MarketMaker::MarketMaker(OrderManagementSystem &oms, int64_t half_spread_ticks, uint64_t order_qty)
    : MBOStrategyBase<MarketMaker>(oms), half_spread_ticks_(half_spread_ticks), order_qty_(order_qty) {}

void MarketMaker::impl_on_book_update(int64_t timestamp_ns, const DataBentoLOB &lob) {
    ...
    if (current_position_ > 0) skew = -half_spread_ticks_ / 2; 
